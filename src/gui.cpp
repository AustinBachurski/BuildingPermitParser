...
Frame::Frame(const std::string& title) : wxFrame(nullptr, wxID_ANY, title, wxPoint(wxDefaultPosition), wxSize(wxDefaultSize), wxDEFAULT_FRAME_STYLE & ~(wxRESIZE_BORDER | wxMAXIMIZE_BOX))
{
	...
	totalBuildingPermitsIssuedBUTTON->Bind(wxEVT_BUTTON, &Frame::copyTotalPermits, this);
	...
	commercialPermitsBUTTON->Bind(wxEVT_BUTTON, &Frame::copyCommercialPermits, this);
	...
	residentialPermitsBUTTON->Bind(wxEVT_BUTTON, &Frame::copyResidentialPermits, this);
	...
	quasiPermitsBUTTON->Bind(wxEVT_BUTTON, &Frame::copyQuasiPermits, this);
	...
	healthCareBUTTON->Bind(wxEVT_BUTTON, &Frame::copyHealthCare, this);
	...
	significantCommercialPermitsBUTTON->Bind(wxEVT_BUTTON, &Frame::copySignificantCommercialCount, this);
	...
	significantCommercialValueBUTTON->Bind(wxEVT_BUTTON, &Frame::copySignificantCommercialValue, this);
	...
	newResidentlaiUnitsBUTTON->Bind(wxEVT_BUTTON, &Frame::copyNewResidential, this);
	...
	sfrTHduplexUnitsBUTTON->Bind(wxEVT_BUTTON, &Frame::copySFR_TH_Duplex, this);
	...
	multiFamilyUnitsBUTTON->Bind(wxEVT_BUTTON, &Frame::copyMultiFamily, this);
	...
	residentialValueBUTTON->Bind(wxEVT_BUTTON, &Frame::copyNewResidentialValue, this);
	...
	significantQuasiBUTTON->Bind(wxEVT_BUTTON, &Frame::copySignificantQuasiCount, this);
	...
	quasiValueBUTTON->Bind(wxEVT_BUTTON, &Frame::copySignificantQuasiValue, this);
	...
	totalValueBUTTON->Bind(wxEVT_BUTTON, &Frame::copyTotalValue, this);
...
void Frame::copyTotalPermits(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getCount(parse.key_TotalBuildingPermits)));
		wxTheClipboard->Close();
		wxLogStatus("'Total Permits Issued' copied to clipboard.");
	}
}

void Frame::copyCommercialPermits(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getCount(parse.key_CommercialPermits)));
		wxTheClipboard->Close();
		wxLogStatus("'Commercial Permits' copied to clipboard.");
	}
}

void Frame::copyResidentialPermits(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getCount(parse.key_ResidentialPermits)));
		wxTheClipboard->Close();
		wxLogStatus("'Residential Permits' copied to clipboard.");
	}
}

void Frame::copyQuasiPermits(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getCount(parse.key_QuasiPermits)));
		wxTheClipboard->Close();
		wxLogStatus("'Quasi Permits' copied to clipboard.");
	}
}

void Frame::copyHealthCare(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getCount(parse.key_HealthCarePermits)));
		wxTheClipboard->Close();
		wxLogStatus("'Health Care Permits' copied to clipboard.");
	}
}

void Frame::copySignificantCommercialCount(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getCount(parse.key_SignificantCommercial)));
		wxTheClipboard->Close();
		wxLogStatus("'Significant Commercial Permits' copied to clipboard.");
	}
}

void Frame::copySignificantCommercialValue(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getValue(parse.key_SignificantCommercial)));
		wxTheClipboard->Close();
		wxLogStatus("'Significant Commercial Value' copied to clipboard.");
	}
}

void Frame::copyNewResidential(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getCount(parse.key_NewResidentialUnits)));
		wxTheClipboard->Close();
		wxLogStatus("'New Residential Value' copied to clipboard.");
	}
}

void Frame::copySFR_TH_Duplex(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getCount(parse.key_NewSFR_TH_DuplexUnits)));
		wxTheClipboard->Close();
		wxLogStatus("'New SFR-TH-Duplex Units' copied to clipboard.");
	}
}

void Frame::copyMultiFamily(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getCount(parse.key_MultiFamilyUnits)));
		wxTheClipboard->Close();
		wxLogStatus("'Multi-family Units' copied to clipboard.");
	}
}

void Frame::copyNewResidentialValue(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getValue(parse.key_NewResidentialUnits)));
		wxTheClipboard->Close();
		wxLogStatus("'Value of all New Residential' copied to clipboard.");
	}
}

void Frame::copySignificantQuasiCount(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getCount(parse.key_SignificantQuasi)));
		wxTheClipboard->Close();
		wxLogStatus("'Significant Quasi Permits' copied to clipboard.");
	}
}

void Frame::copySignificantQuasiValue(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(new wxTextDataObject(parse.getValue(parse.key_SignificantQuasi)));
		wxTheClipboard->Close();
		wxLogStatus("'Significant Quasi Value' copied to clipboard.");
	}
}

void Frame::copyTotalValue(wxCommandEvent& evt)
{
	if (wxTheClipboard->Open())
	{ 
		wxTheClipboard->SetData(new wxTextDataObject(parse.getValue(parse.key_TotalValue)));
		wxTheClipboard->Close();
